...
static void set_color(const struct cli_cmd_entry *pEntry)
{
    char *name;
    (void)pEntry;

    if (!CLI_is_more_args()) {
        CLI_puts("usage: color <name>");
        led_print_colors();
        return;
    }
    CLI_string_ptr_required("name", &name);
    if (!led_set_color_by_name(name)) {
        CLI_printf("unknown color '%s'\n", name);
        led_print_colors();
    }
}

static void show_presses(const struct cli_cmd_entry *pEntry)
{
    (void)pEntry;
    CLI_printf("USR button pressed %lu time(s)\n", (unsigned long)led_button_press_count());
}

const struct cli_cmd_entry my_main_menu[] = {
    CLI_CMD_SIMPLE( "color", set_color, "set the LED color: color <name>" ),
    CLI_CMD_SIMPLE( "presses", show_presses, "show how many times USR was pressed" ),
